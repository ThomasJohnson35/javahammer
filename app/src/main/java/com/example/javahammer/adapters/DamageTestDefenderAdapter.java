...
public class DamageTestDefenderAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    public Context context;
    ArrayList<Model> weaponArrayList;

    public ProfileAdapterListener listener;

    public int selected = 0;

    public DamageTestDefenderAdapter(ArrayList<Model> weaponArrayList) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        switch (holder.getItemViewType()) {
            case 0:
                Model model = weaponArrayList.get(position);
                AddWeaponHolder addWeaponHolder = (AddWeaponHolder) holder;
                addWeaponHolder.name.setText(model.getName());
